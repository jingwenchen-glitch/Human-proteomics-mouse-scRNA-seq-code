...
setwd("/Users/jingwenchen/Desktop/Ph.D/Jin Lab/Proteomics/M_O")
...
# 差异分析（使用limma - 适合小样本）
# 创建样本分组信息
group <- factor(c("M", "M", "M", "O", "O", "O"))

# 构建设计矩阵
design <- model.matrix(~ 0 + group)
colnames(design) <- levels(group)

# 创建 contrasts（比较O vs M）
# 指定比较组
contrast_matrix <- makeContrasts(O_vs_M = O - M, levels = design)
...
# 提取差异结果
diff_result <- topTable(fit2, coef = "O_vs_M", number = Inf, sort.by = "P")%>%
...
M_samples <- colnames(log2_expr)[1:3]
O_samples <- colnames(log2_expr)[4:6]

M_null <- pairwise_diffs(log2_expr, M_samples) %>% as.matrix() %>% as.vector()
O_null <- pairwise_diffs(log2_expr, O_samples) %>% as.matrix() %>% as.vector()
null_vec <- c(M_null, O_null)

# 计算组间差异（between-group）
cross_diffs <- combn(c(M_samples, O_samples), 2, simplify = FALSE) %>%
  keep(~ (.x[1] %in% M_samples & .x[2] %in% O_samples) |
         (.x[2] %in% M_samples & .x[1] %in% O_samples)) %>%
...
p1 <- ggplot(as.data.frame(log2_expr), aes(x = .data[["A_1"]], y = .data[["A_2"]])) +
  ...
  theme_classic() +
  labs(x = "M_1 log2FC", y = "M_2 log2FC") + 
...
p3 <- ggplot(as.data.frame(log2_expr), aes(x = .data[["A_1"]], y = .data[["B_1"]])) +
  ...
  annotate("text", x = min(log2_expr[, "A_1"]), 
           ...
           hjust = 0, vjust = 0, size = 5, color = "black", fontface = "bold") +
  theme_classic() +
  labs(x = "M_1 log2FC", y = "O_1 log2FC") + 
  theme(plot.title = element_blank(),
        legend.title = element_text(size = 14, face = "bold", color = "black"),
        legend.text = element_text(size = 12, face = "bold", color = "black"),
        axis.title = element_text(size = 14, face = "bold", color = "black"),
...
annotation <- data.frame(
  Group = factor(c(rep("M", 3), rep("O", 3)))
)
...
ggplot(pca_all_df, aes(x = PC1, y = PC2, color = Group, fill = Group)) +
  ...
  scale_color_manual(values = c("M" = "blue", "O" = "red")) +
  scale_fill_manual(values = c("M" = "blue", "O" = "red")) +
...
ann_colors <- list(Group = c(M = "skyblue", O = "salmon"))
...
ann_colors <- list(Group = c(M = "steelblue", O ="firebrick"))
# 定义颜色
